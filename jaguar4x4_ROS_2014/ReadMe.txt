...
There are currently 4 data files directly associated with the Jaguar robot: jaguarRecord.dat, commandRecord.dat, commandRead.dat, and commandImage.dat. These each have their own specific purpose, whether it be saving or replaying data for the robot:
1. jaguarRecord.dat is a record file for the current movement command, direction/yaw, latitude, and longitude data of the robot, recorded in that order, with a comma and space between each data entry. This file is updated/written to by the cmdImg_node, refreshed/updated every 20ms. This file does NOT have a timestamp when the info is recorded, but it is in order. Each new line indicates that the robot had been refreshed, therefore new data recorded in a new entry.
2. commandRecord.dat is a record file only for the current movement command. This file is updated/written to by the cmdImg_node, refreshed/updated every 20ms. Every line indicates a new update/new entry; each line has a timestamp (indicating when the command was recieved), the current counter (current given command index), and the actual command itself. The data found in this file may be input into commandRead.dat directly.
...
4. commandImage.dat is used to record the directory of all images that the Jaguar collects while cmdImg_node is active along with the last known movement/keyboard command that the Jaguar recieved when that image was taken. Every line indicates a new update/new entry; each line has an image directory, the current counter, and the respective last command that the Jaguar recieved when that image was taken. The images recorded are in .png format, and their names are the timestamps in which they were recorded. All image files, by default, are currently stored within the jaguar4x4_ROS_2014 source folder, i.e. .../jaguar4x4_ROS_2014/src/camera_images in the catkin workspace. The counter for this .dat file may appear to be missing certain counter values - this is normal, as the Jaguar's camera operates at 30 FPS (or 30 Hz) while the rest of the system operates at 50 Hz, meaning that the camera is somewhat slower, resulting in desync. 
...
The cmdImg_node (cmdImg_node.cpp file) performs all data collection and data processing for the Jaguar robot, outputting formattted data to jaguarRecord.dat, commandRecord.dat, commandImage.dat, and the camera_images folder while it is actively running. It collects GPS, IMU, movement commands, and images.

(Kevin Kongmanychanh)
Check each node's source code and read their comments; they will often tell you information about package and library dependencies that may not be listed here and must be manually downloaded, e.g. OpenCV (specifically opencv2) for the cmdImg_node. They may also provide insight on any problems you may encounter while running them. If that does not help, assuming that it's a node I've written, I've listed my email at the top of each node I've personally written. I suggest any future developers on this team do the same for any newcomers here for any inquiries they may have.

(Kevin Kongmanychanh)
When you add any new packages/dependencies for any nodes that you may add or create, ensure that you update both the CMakeLists.txt file and the package.xml file in the respective source folder, as failing to do so will cause any new packages/directories that you have added to be ignored, despite the fact that they may have been in the right directory. 
